...
class MovieClip extends egret.MovieClip {
	...
	public playFile(name: string,
					playCount: number = 1,
					compFun: () => void = null,
					remove: boolean = true,framesLabel:string|number = "",_loadFun?:()=>void,frameRate?:number): void {
	 ...
	 * 创建主体动画
	 */
	private createBody(framesLabel:string|number = "",frameRate?:number): void {
		...
		//从第一帧开始自动播放
		try {
			this.gotoAndPlay(framesLabel?framesLabel:1, this.playCount);
		} catch (error) {
