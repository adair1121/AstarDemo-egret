...
class VirtualJoystick extends eui.Component{
	...
	private initialPos: egret.Point = new egret.Point( 0 , 0 );
	public constructor() {
		super();
	...
	public childrenCreated(){
		//获取圆环和小球半径
		this.circle.width =  244;
		this.circle.height = 244;
		this.ball.width = this.ball.height = 74;
		this.circleRadius = this.circle.height/2;
		this.ballRadius = this.ball.height/2;
		//获取中心点
		this.centerX = this.circleRadius;
		this.centerY = this.width - this.circleRadius;
		//设置锚点
		...
		this.x = this.initialPos.x = 4;
		this.y = this.initialPos.y = StageUtils.inst().getHeight() - this.height - 1;
	}
	...
	private onTouchBegin(e:egret.TouchEvent){
		...
		// 	return;
		// }
		if( e.stageX > 400 || e.stageY < StageUtils.inst().getHeight() - 350) return ;
		this.x = e.stageX-this.circle.width/2;
		this.y = e.stageY-(this.height - this.circle.height/2);
